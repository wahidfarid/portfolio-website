...
const GithubLastCommit = () => {
    ...
    fetch(`https://api.github.com/users/${user}/repos`)
      ...
      .then(resultData => {
        ...
        let repoData = resultData.reduce((latestRepo, repo) => {
          return new Date(latestRepo.pushed_at) > new Date(repo.pushed_at)
            ? latestRepo
            : repo
        })
        ...
        fetch(
          `https://api.github.com/repos/${user}/${repoData.name}/commits?author=${user}&per_page=1`,
        )
          .then(response => {
            if (!response.ok)
              throw new Error(`GitHub API error: ${response.status}`)
          ...
          .then(commitResultData => {
            if (
              !Array.isArray(commitResultData) ||
              commitResultData.length === 0
            )
              return
  ...
  return (
    <div
      className={`flex flex-col items-start justify-center relative opacity-0 ${commitData.repo ? "transition-all ease-in-out duration-500 opacity-100" : ""}`}
    >
      ...
      <StyledCommitContainer href={commitData.html_url} target="_blank">
        <div className="flex flex-col mb-2">
          ...
          <span className="text-gray-400">
            <Moment fromNow>{commitData.commit?.author.date}</Moment>
          </span>
        </div>
        <p className="pt-2 text-gray-300" style={{ whiteSpace: "pre-line" }}>
